...
- [StringSimilarity][ca.solostudios.stringsimilarity.interfaces.StringSimilarity]: Implementing algorithms define a similarity between
...
- [StringDistance][ca.solostudios.stringsimilarity.interfaces.StringDistance]: Implementing algorithms define a distance between strings
  (0 means strings are identical), like [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] for example.
...
- [NormalizedStringDistance][ca.solostudios.stringsimilarity.interfaces.NormalizedStringDistance]: This interface
  ...
  [NormalizedLevenshtein][ca.solostudios.stringsimilarity.NormalizedLevenshtein] is an example of this.

Generally, algorithms that implement [NormalizedStringSimilarity][ca.solostudios.stringsimilarity.interfaces.NormalizedStringSimilarity]
...
The [MetricStringDistance][ca.solostudios.stringsimilarity.interfaces.MetricStringDistance] interface: A few of the distances are actually
metric distances, which means that verify the triangle inequality \\(d(x, y) <= d(x,z) + d(z,y)\\).
For example, [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] is a metric distance, but
[NormalizedLevenshtein][ca.solostudios.stringsimilarity.NormalizedLevenshtein] is not.
...
A few algorithms work by converting strings into sets of n-grams (sequences of n characters, also sometimes called k-shingles).
The similarity or distance between the strings is then the similarity or distance between the sets.

Some of them, like [jaccard][ca.solostudios.stringsimilarity.Jaccard], consider strings as sets of shingles, and don't consider the number
of occurences of each shingle.
Others, like [cosine similarity][ca.solostudios.stringsimilarity.Cosine], work using what is sometimes called the profile of the strings,
...
The [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] distance between two words is the minimum number of single-character edits
...
### [Normalized Levenshtein][ca.solostudios.stringsimilarity.NormalizedLevenshtein]

This distance is computed as [levenshtein distance][ca.solostudios.stringsimilarity.Levenshtein] divided by the length of the longest
string.
...
An implementation of [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] that allows to define different weights for different
character substitutions.

This algorithm is usually used for optical character recognition (OCR) applications.
For OCR, the cost of substituting P and R is lower then the cost of substituting P and M for example because because from and OCR point of
view P is similar to R.

It can also be used for keyboard typing auto-correction.
Here the cost of substituting T and R is lower for example because these are located next to each other on an AZERTY or QWERTY keyboard.
...
### [Damerau-Levenshtein][ca.solostudios.stringsimilarity.Damerau]

Similar to [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein],
[Damerau-Levenshtein distance][ca.solostudios.stringsimilarity.Damerau] with transposition
(also sometimes calls unrestricted Damerau-Levenshtein distance) is the minimum number of operations needed to transform one string into the
other, where an operation is defined as an insertion, deletion, or substitution of a single character, or a **transposition of two adjacent
characters**.

It does respect triangle inequality, and is thus a metric distance.

This is not to be confused with the optimal string alignment distance, which is an extension where no substring can be edited more than
once.
...
The [Optimal String Alignment][ca.solostudios.stringsimilarity.OptimalStringAlignment] variant
of [Damerau-Levenshtein][ca.solostudios.stringsimilarity.Damerau] (sometimes called the restricted edit distance) computes the number of
edit operations needed to make the strings equal under the condition that **no substring is edited more than once**, whereas the
true [Damerau-Levenshtein][ca.solostudios.stringsimilarity.DamerauLevenshtein] presents no such restriction.
The difference from the algorithm for [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] distance is the addition of one recurrence
for the transposition
operations.

Note that for the [optimal string alignment][ca.solostudios.stringsimilarity.OptimalStringAlignment] distance, the triangle inequality does
...
[Jaro-Winkler][ca.solostudios.stringsimilarity.JaroWinkler] is a string edit distance that was developed in the area of record linkage
...
[Jaro-Winkler][ca.solostudios.stringsimilarity.JaroWinkler] computes the similarity between 2 strings, and the returned value lies in the
interval \\([0.0, 1.0]\\).
It is (roughly) a variation of [Damerau-Levenshtein][ca.solostudios.stringsimilarity.DamerauLevenshtein], where the transposition of 2 close
characters is considered less important than the transposition of 2 characters that are far from each other.
[Jaro-Winkler][ca.solostudios.stringsimilarity.JaroWinkler] penalizes additions or substitutions that cannot be expressed as transpositions.
...
The [longest common subsequence][ca.solostudios.stringsimilarity.LongestCommonSubsequence] (LCS) problem consists in finding the longest
...
The [LCS][ca.solostudios.stringsimilarity.LongestCommonSubsequence] distance between strings X (of length n) and Y (of length m) is
\\(n + m - 2 |LCS(X, Y)|\\),
\\(\\text{min} = 0\\),
\\(\\text{max} = n + m\\)

[LCS][ca.solostudios.stringsimilarity.LongestCommonSubsequence] distance is equivalent
to [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] distance when only insertion and deletion is allowed (no substitution), or
when the cost of the substitution is the double of the cost of an insertion or deletion.

This class implements the dynamic programming approach, which has a space requirement \\(O(m \\times n)\\), and computation cost
\\(O(m \\times n)\\).

In "Length of Maximal Common Subsequences", K.S. Larsen proposed an algorithm that computes the length
of [LCS][ca.solostudios.stringsimilarity.LongestCommonSubsequence] in time \\(O(log(m) \\times log(n))\\).
...
### [Metric Longest Common Subsequence][ca.solostudios.stringsimilarity.MetricLCS]

Distance metric based on [Longest Common Subsequence][ca.solostudios.stringsimilarity.LongestCommonSubsequence], from the notes
"An [LCS][ca.solostudios.stringsimilarity.LongestCommonSubsequence]-based string metric" by Daniel Bakkelund.
...
// => 1 - 6/10 = 0.4
println(metricLCS.distance("ABCDEFG", "ABCDEFHJKL")) // prints 0.4

// LCS: "ABDF" => length = 4
// longest = "ABDEF" => length = 5
// => 1 - 4 / 5 = 0.2
println(metricLCS.distance("ABDEF", "ABDIF")) // prints 0.2
```
...
The distance between two strings is defined as the L1 norm of the difference of their profiles (the number of occurences of
each [n-gram][ca.solostudios.stringsimilarity.NGram]): \\(SUM( |V1_i - V2_i| )\\).
[Q-gram][ca.solostudios.stringsimilarity.QGram] distance is a lower bound on [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein]
distance, but can be computed in \\(O(m + n)\\), where [Levenshtein][ca.solostudios.stringsimilarity.Levenshtein] requires
\\(O(m \\times n)\\)
...
Like [Q-Gram][ca.solostudios.stringsimilarity.QGram] distance, the input strings are first converted into sets of n-grams (sequences of n
characters, also called k-shingles), but this time the cardinality of each [n-gram][ca.solostudios.stringsimilarity.NGram] is not taken into
account.
...
Similar to [Jaccard index][ca.solostudios.stringsimilarity.Jaccard], but this time the similarity is computed as \\(2 * |V1 \\cap V2| / (
|V1| + |V2|)\\).
...
[Ratcliff/Obershelp Pattern Recognition][ca.solostudios.stringsimilarity.RatcliffObershelp], also known as Gestalt Pattern Matching, is a
...
[Ratcliff/Obershelp][ca.solostudios.stringsimilarity.RatcliffObershelp] computes the similarity between 2 strings, and the returned value
...
and [Longest Common Subsequence][ca.solostudios.stringsimilarity.LongestCommonSubsequence].
It was developed to produce a distance measure that matches as close as possible to the human perception of string distance.
